...
function AlumniPage() {
  ...
  const batches = useMemo(
    () => [...new Set(ALUMNI_ALL.map((a) => a.batch))].sort((a, b) => b - a),
    []
  );
      ...
      <div className="flex flex-col md:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-3 rounded-lg border border-border bg-card text-sm outline-none focus:border-primary"
          />
        </div>
        <select
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((a, i) => (
            <div
              key={i}
              className="rounded-lg border border-border bg-card p-3 hover:shadow-md transition"
            >
