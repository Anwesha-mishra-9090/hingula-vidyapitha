  ...
  return (
    <div className="container py-8 space-y-8">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/90 to-primary/60 text-white p-8 md:p-12">
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <a
              href="/about"
              className="px-5 py-2 rounded-lg bg-white text-primary font-semibold hover:bg-white/90 transition"
            >
              Learn More
            </a>
            <a
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/notices" className="text-xs text-primary hover:underline">
              View all →
            </a>
          </div>
          <div className="space-y-3">
            {NOTICES.slice(0, 4).map((notice) => (
              <a
                key={notice.id}
                href="/notices"
                className="flex items-start gap-3 p-2 rounded hover:bg-accent/50 transition"
              >
                <span
                  className={`mt-1 h-2 w-2 rounded-full ${notice.priority === "high" ? "bg-red-500" : notice.priority === "medium" ? "bg-amber-500" : "bg-green-500"}`}
                />
                <div className="flex-1">
                  <div className="text-sm font-medium line-clamp-1">{notice.title}</div>
                  <div className="text-[10px] text-muted-foreground">
                    {notice.category} · {new Date(notice.date).toLocaleDateString()}
                  </div>
                </div>
              </a>
            ))}
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/faculty" className="text-xs text-primary hover:underline">
              View all →
            </a>
          </div>
          <div className="space-y-3">
            {FACULTY.slice(0, 3).map((faculty) => (
              <div
                key={faculty.id}
                className="flex items-center gap-3 p-2 rounded hover:bg-accent/50 transition"
              >
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <a
          href="/ncc"
          className="group flex items-center justify-between p-5 rounded-xl border border-border bg-card hover:shadow-md transition"
        >
