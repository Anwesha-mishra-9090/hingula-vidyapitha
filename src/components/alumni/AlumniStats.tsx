      ...
      {stats.map((stat, i) => {
        ...
        return (
          <div key={stat.label} className="rounded-lg border border-border bg-card p-4 text-center">
            <Icon className="mx-auto h-5 w-5 text-primary"5 text-primary" />
            <div className="mt />
            <div className="mt-2 font-2 font-display text-2xl font-bold">{stat.value.to-display text-2xl font-bold">{stat.value.toLocaleString()}</div>
           LocaleString()}</div>
            <div className="text <div className="text-xs text-muted-xs text-muted-foreground">{-foreground">{stat.label}</divstat.label}</div>
          </div>
          </div>
        );
     >
